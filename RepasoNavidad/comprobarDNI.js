...
var validaDni= new RegExp('^[0-9]{8}[A-Z]{1}$');
var dni;
let cancelar=false;
...
do{
    ...
    if(!validaDni.test(dni.toUpperCase())){
...
}while(!validaDni.test(dni.toUpperCase())&&cancelar==false);

document.write("DNI: "+dni+"</br>");

let numero=parseInt(dni.substr(0,dni.length-1));
        posicionLetra=numero%23;

let letraArray=letras[posicionLetra];

if(letraArray==dni.toUpperCase().charAt(dni.length-1)){
    document.write("Dni válido.")
}else{
    document.write(`Este DNI (${dni}) no es valido, compruebe que este bien escrito`);
}
...
el resto de la división entera del número de DNI y el número 23.
 ...
 A partir del resto de la división, se obtiene la letra seleccionándola dentro de un array de letras. El array de letras es:
    ...
    Con estos datos, elaborar un pequeño script que:- Almacene en una variable el número de DNI indicado por el usuario y en 
    otra variable la letra del DNI que se ha indicado.

    En primer lugar se debe comprobar si el número es menor que 0 o mayor que 99999999. Si ese es el caso, se muestra un mensaje 
al usuario indicando que el número proporcionado no es válido y el programa no muestra más mensajes.
